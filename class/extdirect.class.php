...
class ExtDirect
{
		...
		if (isset($filter)) {
			...
			foreach ($constants as $constant) {
				if (is_array($constant)) {
					$row = new stdClass;
					$row->constant = $constant['name'];
					if (isset($constant['type']) && $constant['type'] == 'json' && !empty(dolibarr_get_const($db, $constant['name'], $entity))) {
						$row->value = ExtDirect::fixJsonString(dolibarr_get_const($db, $constant['name'], $entity));
					} else {
						$row->value = dolibarr_get_const($db, $constant['name'], $entity);
					}
					array_push($results, $row);
				} else {
					$row = new stdClass;
	 ...
	 */
	public static function checkOptionalExist($object, $optional, $visibilities = array(1, 3, 4, 5))
	{
		$results = self::readOptionalModel($object, $visibilities);
	 ...
	 */
	public function selectdolusers($selected = '', $htmlname = 'userid', $show_empty = 0, $exclude = null, $disabled = 0, $include = '', $morefilter = '')
	{
		// phpcs:enable
	...
	*/
	...
	/**
	 * fix and clean JSON string
	 *
	 * @param string $json The JSON string to fix
	 * @return string The fixed JSON string or error message if not fixable
	 */
	public static function fixJsonString($json)
	{
		// remove html entities
		$json = html_entity_decode($json, ENT_QUOTES | ENT_HTML5, 'UTF-8');
		// remove html breaks and line breaks
		$json = str_replace(array('<br />', "<br>"), '', $json);

		// Remove any non-ASCII Characters
  		$json = preg_replace("/[^\x01-\x7F]/","", $json);
