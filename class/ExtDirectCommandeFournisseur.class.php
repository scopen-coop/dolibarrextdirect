					...
					if (!isset($id) || ($id == $line->id)) {
						...
						if ($line->fk_product) {
							...
							if (!empty($supplierProduct->product_fourn_price_id)) {
								...
								if (!isset($mobilid_countstep) && !empty($conf->global->PRODUCT_USE_SUPPLIER_PACKAGING)) {
									$myprod->get_buyprice($supplierProduct->product_fourn_price_id, $line->qty);
						...
						if ($isService || $isFreeLine || !empty($warehouse_id) || ($myprod->stock_reel == 0)) {
							if (($warehouse_id == -1 || $isService || $isFreeLine )) {
								...
								if (isset($supplierProduct) && !empty($supplierProduct->supplier_barcode)) {
									...
									if ($row->barcode && !empty($conf->global->MOBILIDCONNECT_PRODUCT_SHOW_BARCODE)) {
										$row->barcode_html = $myprod->getBarcodeHTML($supplierProduct);
									}
								} else {
